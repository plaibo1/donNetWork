...
const NavContent = () => {
  ...
  return (
    <div className="pb-6 flex flex-col items-center md:flex-row md:items-baseline md:ml-10 md:pb-0 md:space-x-4">
      ...
      <Link href="/about">
        <a className="hover:bg-slate-100 text-gray-800 hover:text-baseColor px-3 py-2 rounded-md text-sm font-medium">
          О нас
        </a>
      </Link>
              ...
              className={`
                ${
                  open
                    ...
                    : "text-opacity-100 text-gray-800"
                }
            ...
            >
              <Popover.Panel className="absolute left-1/2 z-10 mt-3 w-screen max-w-sm -translate-x-1/2 transform px-4 sm:px-0 lg:max-w-3xl">
                <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                  <div
                    className={`bg-white relative grid gap-8 p-7 lg:grid-cols-2`}
                  >
                    {solutions.map((item) => (
                      <a
                        key={item.name}
